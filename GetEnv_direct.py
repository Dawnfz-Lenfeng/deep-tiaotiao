...
class GetEnv:
    def __init__(self):
        ...
        self.start_btn_img = cv2.imread(
            "D:/pythoncode/Deeplearning/pic/start.png", cv2.IMREAD_GRAYSCALE
        )
        self.restart_btn_img = cv2.imread(
            "D:/pythoncode/Deeplearning/pic/restart.png", cv2.IMREAD_GRAYSCALE
        )
    ...
    def _find_start_btn(self, screen_shot_im, find_shot_im):
        ...
        result = cv2.matchTemplate(screen_shot_im, find_shot_im, cv2.TM_CCOEFF_NORMED)
        if result.max() > 0.5:
    ...
    def screen_pic(self, promgram_name, jpg_file, jpg_file2):
        ...
        pydirectinput.click(x=self.start_x, y=self.stat_y, duration=0.25)
        ...
        # 剪裁
        rangle = (
            int(left) * pingmu_suofang,
            int(top) * pingmu_suofang,
            int(right) * pingmu_suofang,
            int(bot) * pingmu_suofang,
        )
    ...
    def touch_in_step(self, action):
        ...
        print("dist:", dist)
        pydirectinput.moveTo(x=self.tiao_x, y=self.tiao_y, duration=0.25)
        ...
        ## 剪裁
        rangle = (
            int(self.left) * pingmu_suofang,
            int(self.top) * pingmu_suofang,
            int(self.right) * pingmu_suofang,
            int(self.bot) * pingmu_suofang,
        )
        ...
        if restart_x == -1:
            ...
            # 失败返回
            pydirectinput.click(x=restart_x, y=restart_y, duration=0.4)
            time.sleep(2)
            state = None
            reward = -1
            done = True
